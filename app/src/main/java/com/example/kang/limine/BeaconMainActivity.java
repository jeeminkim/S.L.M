...
public class BeaconMainActivity extends Activity implements OnInitListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mLayout = findViewById(R.id.beaconmainLayout);
    ...
    public void onInit(int status) {
        myTTS.speak("쿠폰 수령을 시작합니다", TextToSpeech.QUEUE_FLUSH, null);
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if(ActivityCompat.checkSelfPermission(getApplicationContext(), Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                Log.i("BeaconMainActivity", "The location permission (ACCESS_COARSE_LOCATION or ACCESS_FINE_LOCATION) is not granted.");
                this.requestLocationPermission();
            } else {
                Log.i("BeaconMainActivity", "The location permission (ACCESS_COARSE_LOCATION or ACCESS_FINE_LOCATION) is already granted.");
            }
        }
    }
    @Override
        ...
        Snackbar.make(mLayout, R.string.location_permission_rationale, Snackbar.LENGTH_INDEFINITE)
                .setAction(R.string.ok, new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ActivityCompat.requestPermissions(BeaconMainActivity.this, new String[]{Manifest.permission.ACCESS_COARSE_LOCATION}, REQUEST_LOCATION);
                    }
                })
                .show();
    }
    ...
    public void onMonitoringToggleButtonClicked(View v) {
        ...
        if(toggle.isChecked()) {
            Log.i("BeaconMainActivity", "onMonitoringToggleButtonClicked off to on");
            Intent intent = new Intent(this, RecoBackgroundMonitoringService.class);
            startService(intent);
        } else {
            Log.i("BeaconMainActivity", "onMonitoringToggleButtonClicked on to off");
    ...
    public void onRangingToggleButtonClicked(View v) {
        ToggleButton toggle = (ToggleButton)v;
        if(toggle.isChecked()) {
            Log.i("BeaconMainActivity", "onRangingToggleButtonClicked off to on");
            Intent intent = new Intent(this, RecoBackgroundRangingService.class);
            startService(intent);
        } else {
            Log.i("BeaconMainActivity", "onRangingToggleButtonClicked on to off");
